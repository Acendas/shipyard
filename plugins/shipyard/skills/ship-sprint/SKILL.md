...
Any AskUserQuestion the protocol would raise (operational-kind classification, `effort: L` split confirmation) is surfaced by the **shell** after the deep-dive returns — the dispatched agent flags candidates in its return, and the shell runs the user gate. Never run AskUserQuestion inside the dispatched agent. When surfacing a flagged candidate, first render as chat text what the agent found — the task in question, the signal that fired, and the split suggestions or inferred `verify_command` — the agent's return exists only in this context and the user has seen none of it; then ask.
