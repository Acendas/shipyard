...
Once you understand what the user wants, research before challenging. This makes your challenges informed, not generic.

**Constitution check (first)** — read all project rules before anything else:
- Glob `.claude/rules/project-*.md` and read every file found
- Also check `.claude/rules/learnings/` for relevant domain learnings
- Extract: architecture boundaries, banned patterns, naming conventions, domain vocabulary, shared utilities that must be used
- Flag any tension with the feature being discussed — carry these into Phase 1.5b as pre-loaded challenge items. Example: if the constitution bans `any` types and the feature involves a dynamic schema, that's a challenge item. If it defines domain vocabulary ("tenant" not "user"), the spec must use consistent terms.
...
- What's the standard UX pattern? Users have expectations shaped by products they already use — find out what those expectations are
- What open-source implementations exist? Search GitHub for repos that solve similar problems — study their approach, not just their API
- What do users complain about in existing solutions? Search for "[feature] UX problems" or "[product] [feature] frustrating" — learn from others' mistakes
- Best practices for the technical approach (auth flows, payment handling, etc.) — include year for currency (e.g., "auth best practices 2026")
- Known pitfalls or security considerations for this domain
- WebFetch official docs for any mentioned libraries/APIs
...
Once you have a reasonable understanding of the feature, **proactively challenge it** before moving to spec. This is where you earn your keep — surface what the user hasn't thought about.

**Presentation:** Follow the communication design targets (`references/communication-design.md`). Max 3–4 items per AskUserQuestion. If more than 4 open items, batch into groups of 3 by theme (security, data model, UX). For each item: what I found → why it matters → what I recommend. Use the 3-layer pattern for anything genuinely surprising.

Present findings as a compact visual summary before the AskUserQuestion — this lets the user triage at a glance:
```
...
- **NFRs:** Read `${CLAUDE_PLUGIN_ROOT}/skills/ship-discuss/references/nfr-scan.md` — scan for performance, security, data integrity, observability, accessibility, i18n, error UX requirements
- **Failure modes:** Read `${CLAUDE_PLUGIN_ROOT}/skills/ship-discuss/references/failure-modes.md` — build FMEA table, run HAZOP guide words on data flows
- **Pre-mortem:** From `discovery-techniques.md` — imagine the feature shipped and failed, work backward
...
**Iteration loop:**
1. Run the checklist against each feature file
2. If any check fails → fix it. For checks needing user input (e.g., ambiguous requirement), AskUserQuestion to clarify.
3. Re-run checklist on fixed features
4. Max 3 iterations. If gaps remain after 3 → flag them in the discussion summary as "Unresolved — needs follow-up in /ship-discuss [ID]"

Only proceed to Phase 4.95 when the checklist passes or max iterations reached.
