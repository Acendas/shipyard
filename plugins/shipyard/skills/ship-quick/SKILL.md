...
- **When the task description is ambiguous or multiple approaches exist** → render the candidate approaches as chat text (one short paragraph each: approach, tradeoff, recommendation), then AskUserQuestion with the approach names as options. Tradeoffs packed into option strings don't count — the card is too small to carry them. Don't silently pick one — the user came to `/ship-quick` because they wanted speed, not because they wanted you to guess.
