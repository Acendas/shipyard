...
See `references/spec-validation.md` § "Step 9.5" for the full 24-row checklist with fail criteria. **Check 21 (Technical Notes → task mapping)** is load-bearing: a feature's Technical Notes section can name concrete artifacts that never become anyone's deliverable — F002's "Author must write a Playwright spec covering the demo-schema golden path" was such an artifact in the v2.5.0 confedit incident; the sprint shipped with `tests/e2e/` empty because no task owned the deliverable. **Check 24 (`user_flow_probe`)** is also load-bearing: execute blocks at sprint completion if the user-facing proof was not authored at plan time. Iterate up to 3 times, fixing failures and re-running. **This is a no-input self-review loop — run it quietly: do NOT narrate each pass or print the table. Surface only a one-line result** (`→ Quality gate: passed` or `→ Quality gate: N gaps flagged`); the gaps themselves ride into the Step 11 plan summary as "Planning gaps — review during execution". Then proceed to Step 9.7.
