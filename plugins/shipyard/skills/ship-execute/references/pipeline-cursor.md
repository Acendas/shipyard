...
**`sprint` backfills from SPRINT.md when unset.** If neither this call nor the prior cursor carries a `sprint` id, the CLI reads `<SHIPYARD_DATA>/sprints/current/SPRINT.md`'s own `id:` frontmatter and persists that onto the cursor — never fabricated when SPRINT.md is absent or unreadable. Without this, a cursor can spend a whole sprint with no `sprint:` field, and every event it emits tags `sprint:"unknown"`, which breaks the noop repeat-leak detector's and the `wave_N_waiting` reconciliation filter's sprint-scoping (both key off the cursor's sprint id).
...
- `shipyard-data cursor bootstrap-check execute` — the CLI-owned auto-loop eligibility computation. Prints `{"loop_owner":"/loop"|"user","eligible":<bool>,"reason":"…","rearm":<bool>}` and, when eligible, sets `auto_loop_attempted: true` itself and emits `pipeline_loop_bootstrap_eligible`. Replaces the entire model-side eligibility heuristic. `rearm: true` means this eligibility is a re-arm of a `/loop` that went silent after `auto_loop_attempted` was already set — see "Fallback if `/loop` goes silent" below; treat it identically to a fresh `eligible: true`.
...
**A `wave_<N>_dispatch` target stage REPLACES the list, never inherits it.** Any advance whose target stage is a `wave_N_dispatch` writes exactly what that call passes as `pending_subagents` (empty if none) — it does NOT fall back to whatever the prior cursor was carrying. `wave_<N>_waiting` self-loops are the ONLY stage that keeps the merge/carry semantics (a poll tick that doesn't repeat the list must not appear to drop it). Without the dispatch-stage reset, entries from a wave that already finished (and was never re-passed across the dispatch→dispatch hop at a later wave boundary) sit in the cursor with a stale `spawned_at` indefinitely, and the next wave's `wave_N_waiting` reconciliation marks tasks that no longer exist as `needs-attention`.
...
**When nothing looks like it's driving the sprint, the marker says so instead.** A chained direct invocation and a `/loop`-driven tick used to print the identical `▶ TICK COMPLETE` line, so the user could not tell "this will keep going on its own" from "this just stopped and nothing will resume it." Using the SAME liveness signal `bootstrap-check` uses (a recent `pipeline_tick_completed` for this pipeline landing on the stage this tick is advancing FROM), a non-terminal advance instead prints exactly `▶ TICK COMPLETE — no loop is driving this sprint. Re-run /shipyard:ship-execute to continue.` when it finds no such evidence. Echo whichever line the CLI prints — there's nothing else to do with it. This never applies to the four resting paths (terminal advance, `pause`, `escalate`, `noop`), whose own stop-marker contract is unchanged.
