...
6. **Wave-scoped tests + single fix (stage_id: wave_N_tests)** — if `test_commands.scoped` is configured, substitute and dispatch it per the resolution procedure above; else follow the `dispatching-operational-task` playbook with `test_commands.unit` (today's fallback, unchanged). First time tests run for the wave's merged code; the operational task streams progress/failures via Monitor. Failure → ONE re-dispatch via `dispatching-task-loop` with the failing-test list as `continuation_note` (`→ wave_<N>_tests_fix_iter_1`); persistent failure emits `task_status_changed type=wave_tests_failed` and advances. **→ `wave_<N>_verify`.**
...
1. **Full build (stage_id: sprint_full_build)** — if `build_commands.full` configured, follow the `dispatching-operational-task` playbook (catches cross-module compilation errors scoped builds missed). If unconfigured, advance as a pass-through tick. Failure → `shipyard-data cursor pause execute --note "sprint_full_build failed: <why>"` BEFORE AskUserQuestion (so a /loop wakeup can't re-run the whole build and re-ask), then AskUserQuestion. Render the failure evidence first — the failing command and the capture file's error tail — as chat text; content in the capture file or the operational-task return does not count as shown until printed. **→ `sprint_full_tests`.**
2. **Full test suite (stage_id: sprint_full_tests)** — resolve `test_commands.unit`, `.integration`, `.e2e` from config.md FIRST, before dispatching anything (a tier with an empty/absent key is skipped). **Dedupe identical tiers before dispatching**: group the resolved commands by exact string equality; each group maps one-or-more tier names (`unit`/`integration`/`e2e`) onto ONE distinct command, dispatched **once per distinct resolved command**, not once per configured key — a project whose `test_commands.integration` is byte-identical to `.unit` (a real customer's is) must not pay for the same run twice. Note the dedup in the tick's `--note`, e.g. `--note "unit+integration identical, deduped to 2 distinct runs (unit+integration, e2e)"`. For each distinct command, follow the `dispatching-operational-task` playbook with a synthetic `task_id` of `sprint-full-tests-<n>` (n = 1-based index over the distinct commands, in unit→integration→e2e order) so captures land under `<SHIPYARD_DATA>/captures/sprint-full-tests-<n>/run-<iter>.log`. This is the only time the entire suite runs. Persistent failure after the cap → re-dispatch via `dispatching-task-loop` per failing cluster (`→ sprint_tests_fix_iter_1`, K bounded at 1).
   ...
   **Record evidence on a clean pass — per-tier AND aggregate (P1 — closes the redundant re-run at `/ship-review`'s Stage 1a and its quality-gate checks, which check the per-tier keys, not the aggregate one).** When a distinct command's dispatch exits 0, record ONE ledger entry **per tier name that group maps to** — a deduped group covering two tier names (e.g. `unit`+`integration` identical) records TWO entries against the SAME command/exit/capture, one per `--key`, so review doesn't re-run the tier that was deduped away:
   ...
   Once EVERY distinct command has exited 0, ALSO write a summary capture (`<SHIPYARD_DATA>/captures/sprint_full_tests/summary.log`: each distinct command, its own `sprint-full-tests-<n>` capture path, and `exit 0`) and record the aggregate:
   ```
...
3. **Per-feature demo probes (stage_id: sprint_demo_probes)** — the cross-task user-flow proof; catches "all unit tests pass but the integrated feature doesn't work" (the confedit failure mode that motivated this stage in v2.6.0). Read SPRINT.md `features:`. For each feature:
   ...
   - **`demo_probe: skip-with-reason`** with populated `demo_probe_skip_reason` → emit `acceptance_probe_completed feature=<F> probe_type=demo exit_code=0 skipped=true reason=<short>` and continue (Invariant 8: PASS-with-warning). **Also record ledger evidence**, so `/ship-review` Stage 4.8's `verify check --key demo_probe.<F>` finds a fresh entry and doesn't re-ask the same skip question every review: Write a small capture noting the skip (`<SHIPYARD_DATA>/captures/demo_probe_<F>/skip.log`: feature ID, `demo_probe_skip_reason`, timestamp), then `shipyard-data verify record --key demo_probe.<F> --command "skip-with-reason:<demo_probe_skip_reason>" --exit 0 --capture <that file>`. Decision: the recorded command is the skip marker, NOT `feature.demo_probe` itself — this deliberately does not claim any probe command ran, so if a real `demo_probe` is later authored for this feature, the commandHash mismatch invalidates the entry and forces a genuine run instead of a stale skip surviving past it.
   - **Otherwise** → follow the `running-acceptance-probe` playbook (`probe_command: <feature.demo_probe> 2>&1 | tee <SHIPYARD_DATA>/captures/demo_probe_<F>/run.log`, `cwd: <working branch checkout>`, `timeout_seconds: 120`) — tee to a deterministic capture path so a PASS has a file to record against — then emit `acceptance_probe_completed feature=<F> probe_type=demo exit_code=<n> verdict=<PASS|FAIL|TIMEOUT|ERROR>`. **On `verdict=PASS`**, also record ledger evidence: `shipyard-data verify record --key demo_probe.<F> --command "<feature.demo_probe>" --exit 0 --capture <SHIPYARD_DATA>/captures/demo_probe_<F>/run.log` — so `/ship-review` Stage 4.8 can skip re-verification via `verify check` instead of always re-running the probe.
