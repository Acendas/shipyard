...
2. **If `STATUS: COMPLETE`:**
   ...
   - All checks pass → **accept the return with the bundled CLI choke point**: `shipyard-data task accept-return <task_id> sprint=<sprint_id> wave=<wave_number> commit=<sha> --data-dir {{data_dir}}`. This single command pins `shipyard/keep-<task_id>` via `anchor-commit`, emits `task_dispatch_returned status=complete commit_sha=<sha>` with the REAL task id and sha, and marks the task `done` in CLI-owned frontmatter. Pass `--data-dir {{data_dir}}` — the orchestrator already holds the literal data dir it dispatched in the brief, so there's no reason to let the acceptance path re-resolve it (and re-resolution is exactly what can diverge when the orchestrator is itself running in a user worktree). Never emit completion with empty/placeholder fields: null `task`/`commit_sha` is what made the v2.8 audit log blind to which commit belonged to which task, and the terminal gate keys off `task=<id> status=complete`. After acceptance, log the probe tail to the wave's progress.
...
1. **Subagent dies without emitting the event.** The capture file may also be absent. The orchestrator's `wave_<N>_recovery` handler watches per-task spawned_at timestamps. **Presume dead only when all three hold:** `now - spawned_at > max_execution_minutes` (default 60, configurable via task frontmatter) AND no `subagent_completed` event AND no recent `task_loop_iteration` event for that task. A live-but-slow builder still emits `task_loop_iteration` per cycle, so a recent one means "working, not dead" — do not reap it.
   ...
   1. **TaskStop the background agent handle** for that task — use `agent_handle` from `pending_subagents` when available, otherwise use deterministic `agent_name` (`builder-<task_id>`). A presumed-dead subagent may still be a running zombie holding its worktree; stopping it first prevents a late write racing the salvage.
   2. **Salvage the worktree branch WITHOUT merging it.** The builder may have committed real work before dying, but it never passed the gate, so it must not reach the working branch. Anchor the branch tip so the commits survive teardown, then force-remove: `shipyard-data anchor-commit <task_id> <branch-tip-sha> --data-dir {{data_dir}}` (salvage only; do not use `task accept-return`, which would falsely mark an ungated/dead task done) → `git worktree remove --force .claude/worktrees/<id>` → `git branch -D shipyard/wt-<id>` (the `-D` is safe here precisely because the commits are already anchored; this is the one place `-D` is correct — teardown of *integrated* branches still uses `-d`).
