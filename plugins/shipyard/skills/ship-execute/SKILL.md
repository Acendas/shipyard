...
Before re-dispatching after an interruption, main runs `shipyard-data queue requeue-stale --pipeline ship-execute --stage wave_<N>_dispatch`; idempotent stale claims return to `pending`, non-idempotent stale claims remain visible for escalation. Wave completion is gated on `shipyard-data queue list` plus the expected artifacts, not Agent notification delivery.
...
5. **Wave REFACTOR + MUTATE (stage_id: wave_N_refactor)** — dispatch a `general-purpose` subagent with an inline wave-refactor prompt (read the combined wave diff, dedupe + rename + add helpers, run a small mutation check, commit if changes). **Model tier (build):** pass `model: <models.build>` from the `!` context / `<SHIPYARD_DATA>/config.md` if non-empty, else OMIT `model:` (inherit session model); never hardcode a literal. **Effort tier (simplifier):** pass `effort: <agent_effort.simplifier>` from config if non-empty (default `low`), else OMIT `effort:`. Not a wave blocker — on failure emit `task_status_changed type=refactor_failed` and advance. **→ `wave_<N>_tests`.**
