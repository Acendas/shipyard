---
name: ship-debug
description: "Systematic debugging with persistent state that survives /clear and session breaks."
...
3. **Run test** — execute the check (read code, run command, check logs). **Every command run goes through `shipyard-logcap`.** Never invoke a test runner, reproduction script, or diagnostic command directly — wrap it:
   ...
   **For long-running streams** (dev servers, watch mode, `adb logcat`, tail -f equivalents), logcap handles signal forwarding so `Ctrl-C` propagates cleanly, and line-boundary rotation keeps `grep` context intact across rotation. Suggested bounds when invoking `shipyard-logcap run`: unit/integration test runs `--max-size 5MB --max-files 2`; E2E suites `--max-size 20MB --max-files 3`; dev-server / watch mode `--max-size 10MB --max-files 5`; `tail -f` / log streams `--max-size 50MB --max-files 10`.
4. **Record result**:
