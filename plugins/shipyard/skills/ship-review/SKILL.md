...
- **Exit 3 (STALE)** — no fresh proof exists (never recorded, tree changed, TTL expired, capture missing, or any other unevaluable condition — the ledger fails safe to STALE, never to a skip). Run it exactly as before: follow the **`dispatching-operational-task` playbook**; the capability skill captures output through `shipyard-logcap` and returns the structured verdict (PASS/FAIL counts in `LAST_LINES:`). On the tier passing (exit 0), run `shipyard-data verify record --key test_commands.<tier> --command "<same resolved command>" --exit 0 --capture <capture path>` so a later check (a subsequent tick, or the release-approval gate) can reuse it.
...
2. `verify_output` resolves to an existing non-empty file. For new operational tasks this is an absolute `shipyard-logcap path ...` result; for older task files that stored only a logcap capture name, resolve it once with `shipyard-logcap path <verify_output>`. Missing file → **capture lost**, needs re-run.
...
**Patch-task budget (deferral guard).** Before creating another review patch task (any `patch_task_created source=review-*` path above), count the `patch_task_created` events already emitted in this review window (the event log is authoritative — do NOT count from memory). At or above `execution.max_patch_tasks` (config.md, default 5), STOP auto-creating: render the accumulated list and `AskUserQuestion` whether to keep patching, defer the rest to a follow-up sprint, or cut scope. Unbounded auto-patching is deferral-by-another-name; the cap forces the call. Mirrors the execute-side guard and `operational_tasks.max_patch_tasks`.
...
The retro runs in four steps with compaction recovery via `RETRO-DATA.md`'s `step` frontmatter field. Full mechanics — data-gathering source files, throughput computation, IDEA allocation/frontmatter, CLI-owned metrics update, anti-pattern flags — in `references/retro-and-release.md`.
...
Metrics are CLI-owned: `memory/metrics.json` is authoritative and `memory/metrics.md` is generated. Do not Write/Edit either file. Run `shipyard-data metrics record-retro sprint=<sprint-NNN> carry_over="<summary>" bug_rate="<summary>" estimate_accuracy="<summary>" flags="<csv_or_none>"` and include `throughput=<pts_per_hour>` only when throughput was computable. `archive-sprint` records delivered velocity exactly once before rotating `current/`, which keeps the next `/ship-sprint` capacity lookup deterministic. Save key insights to memory.
