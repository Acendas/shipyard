...
## When to invoke (the triggers — do NOT invoke outside these)
...
- `escalation.max_consults_per_sprint` (default 3) — count prior `escalation_consult_dispatched` events for this sprint via `shipyard-context scan-events --tail 200 escalation_consult_dispatched`. If the cap is reached, do NOT dispatch; surface to the user via AskUserQuestion with the consult history summarized.
...
1. **Emit the dispatch event first** (auditability — a consult that leaves no trace can't be capped):
   ...
   ```
   shipyard-data events emit escalation_consult_dispatched pipeline=<ship-execute|ship-review> sprint=<id> trigger=<repeated_fix_failure|integration_gate|uncovered_blocked|critic_deadlock> subject=<task-or-stage-id>
   ```
