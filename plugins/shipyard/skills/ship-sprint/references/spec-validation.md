...
| 24 | **Every feature entering the sprint has a `user_flow_probe:`** | Read each in-scope feature's frontmatter. It must carry `user_flow_probe:` with a valid `kind` (`auto`/`assisted`/`manual`), or an explicit `skip-with-reason` + populated `user_flow_probe_skip_reason`. A legacy scalar `demo_probe:` counts. This is the proof the feature works *for a user* — distinct from per-task `acceptance_probe` (wiring inside one task) and from the full test suite (neither exercises the path a user takes). Gating at plan time is the whole point: `/ship-execute` Step 5.3 blocks sprint completion on this field, so a feature admitted without one builds for hours before the gap surfaces, at which point authoring a probe is a rubber stamp rather than a bar. Fix with `/ship-discuss <feature-id>`. |
