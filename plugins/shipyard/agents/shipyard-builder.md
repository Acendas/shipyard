...
Before returning to the orchestrator — whether you completed the task, got blocked, or are shutting down — you MUST ensure no work is lost:
...
1. Stage and commit them: `git add -A && git commit -m "wip(TASK_ID): partial progress before exit"`
2. Verify: `git status --porcelain` should be empty

**Why this is critical:** Claude Code silently deletes worktree directories when agents exit (bug #29110, #35862). Any uncommitted work is permanently lost. The orchestrator can recover committed work from branches, but cannot recover uncommitted changes from deleted worktrees. This applies to ALL exit paths — normal completion, blockers, shutdown requests, and errors.

**If you cannot commit** (e.g., syntax errors prevent compilation):
- `git stash` as a fallback — stashes survive worktree deletion if the branch still exists
- Report: "WARNING: Work stashed, not committed — check `git stash list` on branch shipyard/wt-[name]"
