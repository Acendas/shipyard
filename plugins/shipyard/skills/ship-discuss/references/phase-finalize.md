...
0. **Gate: every approved feature has a `user_flow_probe`.** Before flipping any status, run `shipyard-data feature check-probes <all approved IDs>` (one call, space-separated). It exits 0 only when every named feature carries a valid `user_flow_probe:` (any `kind`, or a `skip-with-reason` with a populated `user_flow_probe_skip_reason`; a legacy `demo_probe:` counts). **On a non-zero exit, do NOT proceed to step 1** — the CLI names the offending features. This is the omission Phase 3 is supposed to prevent but does not enforce; authoring the probe *here*, before finalize, is what makes the field a real bar rather than a rubber stamp added after a sprint has already built the feature. For each named feature, author its `user_flow_probe:` now (see `references/phase-3-write-spec.md` for the shape and the `kind` decision table — pick `auto` for a callable surface, `assisted`/`manual` for on-device/UI-only flows), render the drafted probe as chat text so the user sees the bar they are approving, then re-run `check-probes` until it exits 0. Only a deliberate `skip-with-reason` (no proof of any kind exists) may pass without a probe command/steps. Do not "approve" a feature past this gate on the promise of a later probe.
...
5. **Mark `.research-draft.md` obsolete via CLI** if it still exists with the current topic — run `shipyard-data draft obsolete-research --topic "<current topic>"` to set `obsolete: true` in its frontmatter.
