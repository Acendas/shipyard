...
2. **Full test suite (stage_id: sprint_full_tests)** — follow the `dispatching-operational-task` playbook per tier (unit/integration/e2e) or combined. This is the only time the entire suite runs. Persistent failure after the cap → re-dispatch via `dispatching-task-loop` per failing cluster (`→ sprint_tests_fix_iter_1`, K bounded at 1). **→ `sprint_demo_probes`.**
3. **Per-feature demo probes (stage_id: sprint_demo_probes)** — the cross-task user-flow proof; catches "all unit tests pass but the integrated feature doesn't work" (the confedit failure mode that motivated this stage in v2.6.0). Read SPRINT.md `features:`. For each feature:
   ...
   - **Otherwise** → follow the `running-acceptance-probe` playbook (`probe_command: <feature.demo_probe>`, `cwd: <working branch checkout>`, `timeout_seconds: 120`), then emit `acceptance_probe_completed feature=<F> probe_type=demo exit_code=<n> verdict=<PASS|FAIL|TIMEOUT|ERROR>`.
