...
1. **Target ~6–8k tokens for the final report.** Leave headroom for tool-call overhead. If approaching that size, cut the lowest-severity findings.
2. **Grep-first, Read-rarely.** For codebase verification, start with Grep. Only Read a source file when Grep has confirmed the symbol exists and you need surrounding logic for a specific finding. **Hard cap: ≤8 codebase Reads per critique** (artifact files don't count). No rabbit holes — if a file is 2+ hops from anything in the artifact, stop and note the uncertainty.
3. **Quote briefly.** Cite `file:line` with at most one line of context. Never paste multi-line blocks.
4. **Cap each finding at ~120 words** (rule / evidence / scenario / fix). If it needs more, it's probably two findings. Tool-call-heavy, prose-light until the final report — narration between tool calls eats the budget.
5. **Stop-early rule.** Once you have 3 solid findings with concrete fixes and priority actions, stop exploring and write the report. Past 5–6 findings is diminishing returns.
6. **Stakes-scaled budget.** `standard`: skip Pass 3, target 3–4 findings, ≤4 codebase Reads. `high`: Pass 3 capped at 2–3 steel-man challenges, target 4–6 findings, ≤8 codebase Reads. Higher stakes mean tighter reasoning, not more volume.
