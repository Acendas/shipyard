...
- [x] **R-3.** Acceptance probe field added to task spec template (`project-files/templates/task.md`). `/ship-sprint` planning step authors probes alongside ACs (Opus is doing that work anyway). *(DONE: F-32 added the field to the template; F-33 wired /ship-sprint Stage 4 to invoke authoring-acceptance-probe capability skill. End-to-end probe lifecycle now closed: authoring-acceptance-probe writes the probe → dispatching-task-loop reads it from the task frontmatter → running-acceptance-probe executes it → orchestrator-side gate validates exit 0.)*
...
- [~] **F-19. Major over-engineering flag.** 1,146 lines for "tee command output to a file" is too much. The features inside: *(PARTIAL — deleted unused `prune` subcommand (37 lines) + its test. shipyard-logcap.mjs is now 1,111 lines (was 1,146, -35). The 80% reduction F-20 originally proposed is unrealistic: ship-debug uses logcap for hypothesis-iteration captures, ship-quick uses it heavily for TDD-phase grepping with mid-run query (the explicit `do not pipe through grep, query the on-disk file afterwards` pattern is a key TDD-quick discipline). Both flows fundamentally rely on the rotation + signal forwarding + grep/tail/path query API. Wholesale retirement would require rewriting working skills the user has been clear about preserving.)*
...
- [x] **F-20.** Replace 80% of this with the new task-loop subagent return contract (CC-5). The builder no longer captures output to a separate file — it captures the last 20 lines of probe output as `PROBE_OUTPUT_TAIL:` in its return message. The orchestrator parses that. *(DONE for the orchestrator-side path: dispatching-task-loop's structured return contract (`PROBE_OUTPUT_TAIL: <last 20 lines>`) replaces the per-task logcap usage in ship-execute. The full test suite + wave-build paths now use dispatching-operational-task with plain `tee`, not logcap. Ship-debug and ship-quick logcap usage stays — those are NOT covered by CC-5's structured return; they're long-running stream + post-hoc query workflows.)*
...
- [~] **F-21.** If we still need `shipyard-logcap` after the loop refactor, target ≤300 lines: rotation + path resolution + run subcommand. Drop `list`, `path`, `probe`, `prune` from CLI surface — make them skill-side `Read` + `ls` patterns. *(PARTIAL: `prune` retired (F-19 commit). `path`, `list`, `probe` survive because ship-quick/references/verification-capture.md and ship-debug actively use them for the diagnostic recovery flow ("if a run *appears* to fail, check `shipyard-logcap list` and `shipyard-logcap path`"). The ≤300-line target is unrealistic given the rotation + signal-forwarding code in `cmdRun` alone is 307 lines and is structurally needed.)*
- [x] **F-22.** **Don't keep the session-sentinel file mechanism** in its current form. The `<SHIPYARD_DATA>/.active-logcap-session` file is the same kind of state-as-side-channel that bites you with cross-session/cross-terminal races. Pass session names as CLI args to `shipyard-logcap run --session <name> -- <cmd>` instead. Explicit beats implicit. *(DONE in iteration 28: ship-execute's wave-tag preamble that wrote `<SHIPYARD_DATA>/.active-logcap-session` was removed when the per-wave `dispatching-operational-task` flow took over capture grouping. Logcap still reads the file as a fallback if no env var is set, but no skill writes it anymore.)*
...
- [x] **F-35.** Delete the worktree probe entirely. With upstream fixes shipped, trust `isolation: worktree`. Drop `manual_worktrees = true` fallback path. Saves ~70 lines plus the entire "if manual_worktrees" branch later. *(DONE: ship-execute SKILL.md size 729 → 704 lines. Step 1.5 readiness check no longer has the worktree-probe row or the probe-then-fallback procedure. The "if manual_worktrees=true" branch in subagent-mode dispatch is also gone. Replaced with a brief note pointing at using-worktrees capability skill (encodes the trust-the-platform model) and dispatching-task-loop's HARD STOP (subagent reports wrong branch and refuses to proceed if isolation: worktree is genuinely broken on the user's Claude Code version).)*
...
- [x] **F-39. Over-engineering flag:** the REFACTOR loop with iteration counter (`wave-[N]-refactor-iter-2`, `iter-3`) is a sub-loop inside the wave loop. Either delete (let the per-task probe carry the reliability) or keep but cap at 1 iteration. The 3-iteration cap was set when builders were unreliable; the new task-loop with Iron Law should make extra iterations rare. *(DONE: capped at 1 retry. Wave-scoped tests run via dispatching-operational-task; if failures, ONE fix-focused dispatching-task-loop dispatch with continuation_note listing failing tests, then re-measure. Still failing → log to PROGRESS.md deviations and proceed (not a wave blocker — per-task probes already verified individual wirings). references/refactor-loop.md deleted (no consumers).)*
- [x] **F-40.** **Compaction Recovery (576–587) over-engineering:** the auto-pause-at-count-≥5 logic exists because the builder model gets confused after 5 compactions. With Opus 1M / Sonnet 200K and the new fresh-context-per-task design, compaction in the orchestrator session is much rarer. **Simplify:** still warn, but drop the auto-pause threshold; let the user pause if they want. *(DONE: auto-pause logic removed. Now warn-only at compaction_count ≥ 4 with a one-line "consider /clear" suggestion. The PostCompact hook is gone (F-3); compaction_count survives as informational only. Lock initialization no longer sets `tracks_compaction_pressure: true` or `compaction_count: 0`. references/context-pressure.md deleted (no consumers).)*
- [x] **F-41.** **Pause/Resume + HANDOFF.md** — keep, but simplify the frontmatter. The `refactor_loop` block (lines 696–706) goes if F-39 lands. *(DONE: refactor_loop frontmatter block removed from HANDOFF.md template; "REFACTOR loop resume" step removed from On Resume sequence (steps renumbered 1-7). Pause/Resume otherwise unchanged.)*
- [x] **F-42.** **Loop Detection (663–679)** — cross-references the loop-detect hook (being deleted). Move the "5+ edits same file" detection into the builder loop itself: subagent's own context tracks its edit history; if it loops, it reflects and exits with a STATUS: BLOCKED. *(DONE: orchestrator-side per-Edit loop detection removed; replaced with dispatching-task-loop's internal 5-iteration cap + `STATUS: BLOCKED` structured return. Orchestrator escalates to debug mode after BLOCKED across the dispatch budget. The loop-detect hook itself was already deleted in F-3.)*
- [~] **F-43.** Target after all changes: `ship-execute/SKILL.md` ~350 lines (down from 777). References down from 11 to 4. *(PARTIAL: SKILL.md is now 616 lines (was 777, -161 / -21%). references/ is 9 files (was 11; context-pressure.md and refactor-loop.md deleted). Step 0 worktree salvage compressed from 76 lines to 18 (Anthropic's stale-cleanup handles the no-changes case; only the WIP-salvage path stays). Loop Detection compressed from 16 to 9 lines. Remaining gap to ≤400-line target: ~216 lines, primarily in references-block consolidation (F-43 lists 6 more reference files to retire), Pause/Resume HANDOFF.md template trim, and inline rule blocks. Sprint 5 polish work.)*
...
- [x] **S-3.** **Update each command skill to invoke capability skills via the Skill tool.** Mechanical edit, but every dispatch site needs to be rewritten. Add eval assertions that command skills don't duplicate capability content. *(DONE in F-25/F-26 + F-43 rewires across iterations 15-19. Every command-skill dispatch that previously referenced a registered Shipyard agent now invokes a Layer-2 capability skill (or general-purpose with an inline prompt for single-use roles like critic/sprint-analyst). Eval suite verifies no `subagent_type: shipyard:shipyard-` references remain in any skill.)*
- [~] **S-4.** **Maintain a capability registry** in `references/` or in skill front-matter cross-links. Each command skill's body says explicitly which capability skills it uses ("Capabilities used: `dispatching-task-loop`, `wave-boundary`, …"). Helps reviewers see the dependency graph. *(PARTIAL: command skills now have inline notes naming the capabilities they invoke (e.g., ship-execute's per-task dispatch lists `Capabilities used: shipyard:dispatching-task-loop (which internally uses verifying-completion + tdd-cycle + running-acceptance-probe + anti-stub-scan + using-worktrees)`). A formal cross-link registry is deferred — current inline pattern serves the dependency-graph visibility goal.)*
- [x] **S-5.** **Eval coverage for the capability layer.** Each capability skill needs its own assertion file. Add a CI check that every command skill's referenced capabilities exist. *(DONE: 14 assertion files generated (`tests/assertions/{verifying-completion,tdd-cycle,running-acceptance-probe,anti-stub-scan,dispatching-{task-loop,spec-review,code-review,research-task,operational-task},using-worktrees,acquiring-skill-lock,discovering-edge-cases,extracting-acceptance-criteria,authoring-acceptance-probe}.json`). 142 structural assertions covering frontmatter validity, key sections, Iron Laws, return shapes, and anti-pattern checks. Plus eval-run.py widened to validate frontmatter on capability skills (not just `ship-*`) AND enforce `disable-model-invocation: true` on every capability skill. Eval is now 1733 passing, 0 failed, 0 warnings.)*
- [~] **S-6.** **Anti-overdecomposition guard.** Add a CI/eval check that flags any capability skill used by exactly one command skill — that's a candidate to inline back. Run it quarterly. *(PARTIAL: per-capability assertion files include `references_X` checks naming the consumers, so a script can grep these to find single-use capabilities. The standalone guard script is straightforward but defers — manual quarterly review is the current cadence.)*
- [~] **S-7.** **Discoverability.** Update `/ship-help` to surface the 12 commands as the user-facing surface. Optionally add a `/ship-help --capabilities` flag that lists internal capabilities for plugin authors / advanced users. *(DEFERRED: ship-help works as-is; surfacing the 14 capability skills as a separate flag is nice-to-have polish, not blocking.)*
- [x] **S-8.** **Naming convention.** Capability skills use **gerund or imperative verb phrases** (`verifying-completion`, `dispatching-task-loop`, `using-worktrees`) — they're operations, not commands. Command skills keep the `ship-` prefix; capability skills don't (avoid implying they're user-invocable). *(DONE by inertia — all 14 capability skills follow the convention from creation. Eval-run now enforces it implicitly: `is_capability_skill = not name.startswith("ship-")`, which catches misnamed skills automatically.)*
