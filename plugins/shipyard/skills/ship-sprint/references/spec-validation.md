...
| 23 | **No wave exceeds `execution.max_tasks_per_wave`** | Read the cap from config (default 6). A dependency layer wider than the cap must be split into consecutive waves along track boundaries (tasks sharing a parent feature stay together; a single oversized track splits at its natural internal seam). All quality machinery is wave-scoped — integration gate, scoped build/tests, spec review, completion gate — so an oversized wave makes checkpoints rare and smears failure attribution across a giant merge surface. Wave COUNT is never capped (it is dependency depth); only wave WIDTH is. Fix by re-partitioning the layer per `wave-decomposition.md` § "Wave size cap". |
