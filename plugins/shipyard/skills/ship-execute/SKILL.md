...
- **Cursor** — `shipyard-context sprint-execution` reads the cursor through the shared CLI parser and emits `SHIPYARD_CURSOR_*` fields; `shipyard-context cursor-state execute` is the mid-session re-read surface. Mutations go through `shipyard-data cursor advance|set|resume|pause|escalate|noop execute …`. See the tick-exit contract below and [references/pipeline-cursor.md](references/pipeline-cursor.md) for the full schema, stage map, and event vocabulary. Read that reference before changing any per-tick wiring.
...
**Goal driver contract.** `/goal` is the only autonomous liveness driver for this command. It may re-enter `/ship-execute` until the Shipyard cursor reaches terminal, paused, or escalated state. `/goal` is never state authority; the cursor, event log, queue ledger, verification ledger, and artifacts are. Never invoke `/goal` from a spawned Agent, and never create a nested goal when this command is already running inside an active goal. A goal tick runs the same stage handler as a direct invocation, advances the cursor, and stops/continues only according to the CLI-owned cursor result.

**Flat-worker contract.** `/ship-execute` is the sole orchestrator. It may spawn builder/test/recovery agents directly, but those agents are leaf workers: they claim at most one queued item via `shipyard-data queue claim`, write the required artifact, call `shipyard-data queue complete|fail`, and return only task id, status, and artifact path. A worker must never spawn another agent, invoke `/goal`, own cursor advancement, or decide wave completion. Completion is proven by queue state plus artifacts, not Agent notification delivery.
...
- `--isolation true|false` → Force whether builders run in git-worktree isolation for this invocation, overriding `execution.isolation` in config.md (default `worktree`). Accepts either vocabulary — `true`/`worktree`/`on` all mean isolate, `false`/`none`/`off` all mean in-place (resolved via `shipyard-data resolve-isolation --flag <value>`). `--isolation false` runs **sequentially in-place on the working branch** (no worktrees) — the deliberate choice for a heavy build where a warm checkout beats parallelism; it forces the `solo` dispatch shape and, combined with `--mode task|track`, coerces to sequential with a one-line note (no-isolation is sequential-only — see Step 2's Isolation resolution rule).
- `--single-tick` → Force one stage only for deterministic testing/debugging; otherwise `/goal` may continue re-entering until terminal/paused/escalated
...
The `shipyard-context sprint-execution` pre-exec block above acquires the execution lock and returns the execution context bundle in one CLI call, including `SHIPYARD_SPRINT_ID=<id-or-unknown>`. If that context contains `SHIPYARD_LOCK_ACQUIRED=false`, echo the `SHIPYARD_LOCK_BLOCKED:` `⛔` block text verbatim as the entire response and STOP — do not load any further context, do not call other tools. The bundled command's cross-lock guard HARD-BLOCKS `ship-sprint` and other planning work, but allows the narrow `ship-discuss` + `ship-execute` pair so discussion can author future/backlog work while execution owns the active sprint. If stdout includes `SHIPYARD_LOCK_CROSS_ALLOWED=ship-discuss+ship-execute`, proceed normally; `/ship-discuss` is responsible for treating active-sprint specs/tasks as read-only. The lock path also handles cleared-sentinel detection, 2h-stale recovery, and corrupt-JSON recovery — echo any `SHIPYARD_LOCK_NOTICE:` line, then proceed. Do not run a separate lock-acquire call; the bundled context command already did it.
...
`/ship-execute` is goal-driven: every invocation reads a persistent **pipeline cursor**, dispatches to the matching stage handler, and advances the cursor for the next tick. Full schema, stage map, terminal protocol, and event vocabulary live in [references/pipeline-cursor.md](references/pipeline-cursor.md).

**Starting the autonomous run.** Use `/goal` with `/ship-execute` as the objective when the user wants the sprint to continue without manual re-entry. A plain `/ship-execute` invocation runs the current bounded stage, prints the CLI marker, and stops; the marker is a contract for `/goal`, not proof that another local command invocation exists.
...
The CLI validates the transition against the stage graph, runs the stale-cycle guard + terminal-evidence gate in-process, appends `pipeline_tick_completed` (or `pipeline_terminal`), auto-emits the new stage's `pipeline_tick_started`, re-renders PROGRESS.md, and prints the marker (for terminals the `/goal should stop` marker is guaranteed the FINAL line). **Echo the CLI's stdout as the final lines of your tick output** — never emit the pipeline-lifecycle events yourself and never print your own markers. With `--single-tick`, exit after echoing; otherwise `/goal` owns re-entry. Because of this contract, each stage below states only its `→ <next-stage>` target plus any stage-specific `k=v`.

**A non-terminal advance's marker tells `/goal` to continue.** The normal marker is `▶ TICK COMPLETE — ... /goal continues.`. Terminal, pause, and escalation markers tell `/goal` to stop.
...
**Pause-before-blocking-ask (load-bearing).** A tick NEVER exits with a pending question and no marker. Any stage that must block on `AskUserQuestion` first runs `shipyard-data cursor pause execute --note "<question pending: …>"`, which sets the resume surface and prints the stop marker. Already wired at `sprint_full_build`, `sprint_demo_probes`, and the readiness anomaly prompt.
...
2. Use the `SHIPYARD_CURSOR_*` fields from the bundled `shipyard-context sprint-execution` block. Do not Read or parse `EXECUTE-CURSOR.md` directly; if cursor state must be refreshed mid-invocation, run `shipyard-context cursor-state execute`.
   - **`SHIPYARD_CURSOR_TERMINAL=true`, `SHIPYARD_CURSOR_STATUS=escalated`** → an escalated (recoverable) halt, NOT a completed sprint. Run `shipyard-data cursor noop execute` (the CLI detects the escalated terminal, does NOT emit a noop or arm the leak alarm, and prints the resume hint); echo it. Tell the user the sprint is resumable via `shipyard-data cursor resume execute` once the cause is fixed, then STOP.
   - **`SHIPYARD_CURSOR_TERMINAL=true`** (any other status) → a leaked wakeup against a finished cycle. Run the No-op sweep below and exit.
   - **`SHIPYARD_CURSOR_STATUS=paused`** → **goal-inert.** `/goal` must NEVER auto-resume a paused sprint — pause is a deliberate user stop, and only the user un-pauses it. Run `shipyard-data cursor noop execute` (the CLI emits `pipeline_terminal outcome=noop reason=awaiting_user_paused`, prints the pause note + the "resume is a USER decision (`shipyard-data cursor resume execute`)" hint + the stop marker; a repeated noop against the same paused sprint trips the repeated-noop warning), echo it, and STOP. Run `shipyard-data cursor resume execute` (flips to `in_progress` at the recorded stage, then dispatch to `SHIPYARD_CURSOR_STAGE` using `SHIPYARD_CURSOR_NOTE` as resume context) ONLY when the user explicitly asked to resume — invoked `/ship-execute` saying resume/continue, or answered a surfaced question. (See "Recovery & resume".)
   - **`SHIPYARD_CURSOR_PRESENT=true`, `SHIPYARD_CURSOR_TERMINAL=false`, `SHIPYARD_CURSOR_STATUS=in_progress`** → dispatch to the handler for `SHIPYARD_CURSOR_STAGE`.
   - **`SHIPYARD_CURSOR_PRESENT=false`** → fresh start. Begin at Pre-flight; the first `shipyard-data cursor advance execute preflight` creates the cursor.
3. **No-op terminal sweep (MANDATORY — load-bearing for goal safety).** Even if step 2 passed as non-terminal, verify the sprint is alive against all THREE conditions below. If ANY hold, run the sweep and exit. NEVER skip it. The CLI's in-process stale-cycle guard also refuses a non-terminal advance when there is no live sprint.
4. Run the chosen stage's handler, then advance per the tick-exit contract.

### No-op terminal: already-completed sprint (+ repeated-noop self-detection)
...
1. Run `shipyard-data cursor noop execute [sprint=<id>] [reason=<sprint_already_complete|cursor_already_terminal>]` and echo its output. The CLI emits `pipeline_terminal outcome=noop` FIRST, runs repeated-noop detection, and on the 2nd noop for the same sprint/reason emits `pipeline_repeated_noop_detected` and prints the hard `⛔ REPEATED NOOP …` marker; otherwise it prints `▶ CYCLE COMPLETE — sprint already complete. /goal should stop.`

### Goal vs Single-Tick Dispatch Contract
...
2. **Pre-flight gates.** `/ship-execute` runs /goal-shaped by default (sprint-start to sprint-done, halting only on documented escalation contracts). Eight pre-flight gates refuse entry when a known-ambiguous condition exists so /goal never compounds a structural problem. Run each in order; first failure emits `shipyard-data events emit sprint_goal_preflight_failed gate=<name> sprint=<id>` and halts with an actionable message. Gate names, checks, rationale, and fix copy: [references/goal-mode-preflight.md](references/goal-mode-preflight.md).
   ...
   **`every_feature_has_user_flow_probe` runs in EVERY mode, including `--task` and `--hotfix`** — the other seven skip on those. Run it as a CLI gate: `shipyard-data feature check-probes <all SPRINT.md feature IDs>` (exit 3 = fail, names the offenders); do not re-derive it by reading each frontmatter in-model. It is the same `check-probes` verb `/ship-discuss` Finalize and `/ship-sprint` Check 24 call, so the three stages agree by construction. It is checked here rather than at Step 5.3 because a missing probe discovered at sprint-completion means the sprint already built: the fix (`/ship-discuss <F>`) is one command now and a wasted sprint later.
...
- **Output capture.** Test/build/E2E verification runs are dispatched via `dispatching-operational-task`, which captures stdout+stderr through `shipyard-logcap run <task_id>-run-<N> --filter ... --exit-file <SHIPYARD_DATA>/captures/<task_id>/run-<N>.exit`. The task's `verify_output:` is the absolute path returned by `shipyard-logcap path <task_id>-run-<N>`. Don't run verification commands directly in this session — delegate.
- **Communication.** Quiet by default (above): between gates, only the one-line stage/wave markers, the wave progress bar, and background-dispatch banners reach the chat — no step-by-step narration and no forwarding subagent chatter. Blocker reports and decisions (at a gate) use the 3-layer pattern (one-liner / context / options); keep under 100 words; always recommend a default. Full guide: `${CLAUDE_PLUGIN_ROOT}/skills/ship-discuss/references/communication-design.md`.
...
**Also:** `shipyard-data ensure-worktree-baseref` — sets `worktree.baseRef: "head"` in the project's `.claude/settings.json` (idempotent atomic JSON read-merge-write that preserves every other key — never a model hand-edit, which is the corruption class that welded `status: done` + `effort: M` into `status: doneeffort: M`). Verifying it every sprint is self-healing and independent of one-time setup. It is the backstop for when the `WorktreeCreate` hook does not fire (native creation would otherwise fork from `origin/<default>` and drop earlier waves' local commits).

**Also:** `shipyard-data ensure-shared-caches` — materializes the `shared_caches:` config map (package-manager download-cache env vars) into `.claude/settings.json` `env` via the same atomic read-merge-write, so every builder (parent or worktree subagent) inherits a warm download cache. No-op when `shared_caches` is empty (the default), so this is safe to run unconditionally every sprint.
...
**Before the AskUserQuestion, run `shipyard-data cursor pause execute --note "readiness anomaly: <codes>"`** (pause-before-blocking-ask — `/goal` must stop instead of re-running readiness and re-asking; the pause note is the resume surface). The `using-worktrees` capability skill encodes the trust-the-platform model; `dispatching-task-loop`'s HARD STOP catches genuinely-broken isolation.
...
Per-wave stage IDs: `wave_<N>_dispatch`, `wave_<N>_redispatch_iter_<K>` (K∈{1}), then `wave_<N>_boundary` → `_build` → `_refactor` → `_tests` → `_verify` → `_gate` (Step 4). After the last wave's gate → `sprint_refactor` (or `sprint_full_build` under `refactor_scope: wave`) (Step 5).

**Collapse pass-through stages into one tick (speed, load-bearing for cost).** A boundary stage with nothing configured to run — `wave_<N>_build` with no `build_commands`, `wave_<N>_refactor` under the default `execution.refactor_scope: sprint` — still costs a full `/goal` re-entry if ticked on its own. Collapse those into the preceding advance with `--via`:

```
shipyard-data cursor advance execute wave_<N>_tests --via wave_<N>_build --via wave_<N>_refactor
```

Each hop still emits its own `pipeline_tick_completed` / `pipeline_tick_started`, so the terminal gate and wave invariants (event-log readers) see an identical trail; one illegal hop refuses the whole chain atomically. **Collapsible stages are a config-dependent CLI allowlist** — anything that dispatches an agent, runs a command, or produces a verdict is refused, so this is enforcement rather than the earlier prose request to collapse only no-op stages. Full contract: [references/pipeline-cursor.md](references/pipeline-cursor.md) § "Collapsing pass-through stages".
...
**A declared or forced mode is honored — full stop.** Execute the declared/forced mode. Do NOT silently override it, do NOT route to `escalating-to-thinker` (mode selection is not one of its seven triggers), and do NOT ask the user to re-confirm a mode they already declared. The incident behind this rule was a shell that silently dispatched `task` for a sprint declaring `track`; the fix is to honor the declaration, and honoring it needs no interrupt. Isolation is **per-mode, not universal**: `task`/`track` isolate each concurrent builder in its own worktree and integrate through the branch-based `verify-wave-integrated` gate; `solo` runs **sequentially in-place on the working branch with no worktree** — sequential work has no concurrent writer to race, so there is nothing for isolation to protect and the integration gate is vacuously satisfied (zero `shipyard/wt-*` branches). A forced mode that is genuinely unavailable in this session falls back with a one-line note, which is a statement, not a question.

**Isolation resolution (`--isolation`).** Do NOT hand-derive this from prose — call the CLI so resolution has one source of truth: `shipyard-data resolve-isolation [--flag <value>]`, passing the `--isolation` argument as `--flag` when the user supplied one (it accepts `true|false|worktree|none`). It prints exactly `worktree` or `none`, applying the precedence flag > `execution.isolation` in config.md > default `worktree`. Capture that once and carry it as the wave's `isolation` decision. When it prints **`none`**, dispatch **sequentially in-place** regardless of task count — this is the `solo` dispatch shape, chosen deliberately for a heavy build where a warm canonical checkout beats parallelism. No-isolation is **sequential-only by construction**: parallel builders on a shared checkout would clobber each other's edits and there is no per-task `shipyard/wt-*` branch for the wave-integration gate to rebase. If `--isolation false` is combined with `--mode task|track`, **coerce to sequential in-place and emit a one-line note** (`Isolation off forces sequential execution; running solo-shape despite --mode <m>.`) — a statement, not a question. When isolation is on, `task`/`track` behave exactly as today.
...
- **`track`** (alias `team`) — **experimental opt-in only; never auto-selected.** Native nested Agent behavior has known completion-routing and ownership risks, so this mode is not the default fast path. Prefer flat `task` mode unless the user explicitly requested track-mode soak. A forced/declarative `track` run must first read `${CLAUDE_PLUGIN_ROOT}/skills/ship-execute/references/track-mode.md`; if the platform cannot satisfy its probe/fallback contract, fall back to flat queued workers with a one-line note rather than spawning nested builders.

#### Proactive Queue Dispatch (task mode fast path)

For normal `task` mode waves, main enqueues the wave's tasks before spawning workers:

```
shipyard-data queue enqueue --pipeline ship-execute --stage wave_<N>_dispatch --input <wave-work-items.json> --require-isolation <resolved worktree|none>
```

**Include each item's `effort`** (from the task file's `effort:` frontmatter) in the work-items JSON. Under the default `execution.dispatch_order: critical_path`, `queue enqueue` sorts the wave longest-effort-first (XL→L→M→S, task-id tie-break) so the long pole overlaps the short work instead of running alone in the wave's tail — `claim` hands out array order, so enqueue order *is* dispatch order. This is CLI-enforced, not a prose request: omitting `effort` just sorts that item last, it never fails the enqueue. Merge order at the wave boundary stays task-ID order regardless.

Then spawn up to `execution.max_parallel_agents` identical leaf workers. Each worker prompt must say:

```
Claim exactly one item:
shipyard-data queue claim --pipeline ship-execute --stage wave_<N>_dispatch --worker <worker-id>

If claimed=false, return immediately. If claimed=true, perform only that item
directly in this worker context, write the expected artifact, then call either:
shipyard-data queue complete <task-id> --pipeline ship-execute --stage wave_<N>_dispatch --worker <worker-id> --result <absolute-artifact-path>
shipyard-data queue fail <task-id> --pipeline ship-execute --stage wave_<N>_dispatch --worker <worker-id> --reason "<short reason>"

Never spawn another Agent. Never invoke /goal. Never call a capability skill as a nested dispatcher. Return only task id, status, and artifact path.
```
...
- **`kind: feature`** or **absent** → standard feature-builder work item (documented below). Main reads `dispatching-task-loop` once to copy its builder discipline and gate requirements into the worker brief; the worker does not call that capability skill.
...
For each feature task, main constructs one queued work item and one leaf-worker prompt from the `dispatching-task-loop` playbook. Read that playbook for the builder discipline, the structured-return contract, the orchestrator-side gate (sha verify + PROBE_EXIT:0 check + anti-stub-scan), the iteration cap, and the single-redispatch rule, then inline the relevant requirements into the worker prompt. Do NOT route the worker through `dispatching-task-loop` or any other wrapper skill; the worker is already the spawned Agent.
...
| `isolation` | `on` or `off` — the resolved isolation decision (Step 2: flag > `execution.isolation` > default `worktree`). `on` → spawn with `isolation: "worktree"`; `off` → omit the arg, run in-place sequentially. Surfaced to the worker as the `Isolation:` brief line so its branch self-check knows which branch to expect. |
| `worktree_path` | null whenever isolation is off (solo, or `task`/`track` coerced off by `--isolation false` / `execution.isolation: none`); absolute worktree path when isolation is on in task mode (track mode's nested per-task dispatch follows this same contract via the track coordinator — see `references/track-mode.md` — rather than being called directly from this table) |
| `sprint_id` | `id:` from SPRINT.md frontmatter |
| `wave_number` | current wave number from the cursor |
| `dispatch_mode` | `queue` for wave-dispatch and sprint-test-fix-redispatch; `sync` for `--task`/`--hotfix` |

When isolation is on, each task-mode leaf worker is spawned with `isolation: "worktree"` (per `using-worktrees`); when isolation is off — solo, or `task`/`track` coerced off by `--isolation false` / `execution.isolation: none` — the worker is spawned with **no** `isolation:` argument and runs in-place on the working branch, sequentially. The worker writes a structured JSON artifact (`task`, `status`, `commit_sha`, `probe_exit_code`, `output_tail`, `capture_file`, `escalation_code`) at the queue item's `expected_artifact` path (relative paths resolve under `<SHIPYARD_DATA>/sprints/current/`) and reports it through `shipyard-data queue complete|fail` with the same `--pipeline` and `--stage` used for claim. The queue CLI refuses completion when the artifact is missing, not JSON, for a different task, missing required fields, carries a missing/relative `capture_file`, or does not match `expected_artifact`. **Do not parse freeform Agent output.**

Capabilities whose rules are inlined into the worker brief: `dispatching-task-loop` (builder discipline and return contract), `verifying-completion`, `tdd-cycle`, `running-acceptance-probe`, `anti-stub-scan`, `using-worktrees`.

**Background dispatch.** Queue items own task assignment; workers own execution. Spawn leaf workers with deterministic names and queue claim prompts, record worker ids in `pending_subagents`, and advance `→ wave_<N>_waiting`. Each worker records its structured result through `shipyard-data queue complete|fail`. The queue ledger and result artifacts are authority; Agent notification delivery is advisory only.

The orchestrator never reads the Agent tool's return value in queue mode — the structured-return contract is preserved via the result artifact path recorded in the queue item (read it as JSON; never regex for `STATUS:` lines). `/goal` re-entry is driven by cursor state; queue events are audit/progress signals, not the sole wake mechanism.
...
- **`kind: feature`:** verify key files exist + commits present (`git log --grep="TASK_ID"`); **item completeness** — if Technical Notes lists discrete items (e.g., "migrate 8 calls"), grep the diff for each, <100% covered → re-dispatch with the missing list as `continuation_note`; **no-commits salvage** — dirty worktree → WIP-commit + re-dispatch with a continuation note, clean worktree (did nothing) → `shipyard-data task set-status <id> approved`; single re-dispatch per task per wave, persistent failure → `shipyard-data task set-status <id> needs-attention --reason "salvage_failed"`; **in-place clean-tree gate (isolation off only)** — after each COMPLETE task, run `shipyard-context check-dirty-tree`; non-empty means the builder left the shared working tree dirty (partial edits that would bleed into the next sequential task). WIP-commit the residue onto the working branch (`git add -A && git commit -m "wip(<TASK_ID>): salvage in-place residue"`) before dispatching the next task — never start the next in-place task on a dirty tree. (In worktree mode this is a no-op: each builder's tree is its own worktree, covered by `check-dirty-worktrees`.); **effort-gated single-task spec check** — for `effort: M|L|XL`, follow the `dispatching-spec-review` playbook `scope: "task"` (skip `effort: S`); **merge** — rebase + ff-merge the worktree branch, remove worktree, delete merged branch.
- **`kind: operational`** / **`kind: research`:** the capability skill's gate is authoritative; record the verdict and advance.

**→ `wave_<N>_waiting`** (queue mode) — see the waiting handler. In sync mode (`--task`/`--hotfix` only): all COMPLETE → `wave_<N>_boundary`; any BLOCKED → `wave_<N>_redispatch_iter_1 iteration=<n+1>`.

#### Wave waiting handler (stage_id: wave_N_waiting)

`pending_subagents` is a progress mirror for the worker ids spawned this wave; the queue ledger is the authority. Do not infer completion from Agent notifications. Track mode maintains a separate per-track handle/timeout list only when the experimental track coordinator is explicitly used; flat task mode uses the queue list below.

When `/goal` re-enters at `wave_<N>_waiting`:

1. Run `shipyard-data queue requeue-stale --pipeline ship-execute --stage wave_<N>_dispatch`, then `shipyard-data queue list --pipeline ship-execute --stage wave_<N>_dispatch`.
2. Interpret queue statuses:
   - `pending` → spawn more leaf workers up to `execution.max_parallel_agents`, then self-loop `wave_<N>_waiting`.
   - `claimed` with an unexpired `deadline_at` → still in flight; self-loop `wave_<N>_waiting`.
   - `stale` or failed non-idempotent stale item → if replay is explicitly safe, run `shipyard-data queue retry-stale <task-id> --pipeline ship-execute --stage wave_<N>_dispatch --reason "<why replay is safe>"` and self-loop; otherwise run `shipyard-data queue park-stale <task-id> --pipeline ship-execute --stage wave_<N>_dispatch --reason "worker_stale_non_idempotent"`, mark the task `needs-attention` with reason `worker_stale_non_idempotent`, emit `task_dispatch_returned status=blocked`, then continue after removing it from the pending mirror.
   - `failed` → route through the blocked/escalation handling from the task-loop gate, emit `task_dispatch_returned status=blocked`, and do not redispatch more than once.
   - `complete` with `artifact_status: COMPLETE` → queue for `wave_<N>_recovery`; the artifact at `task.result` is the only accepted return surface.
   - `complete` with `artifact_status: BLOCKED` → route through the blocked/escalation handling from the artifact JSON; the worker returned a valid contract, but not a passed task.
3. If any queue item is `pending` or `claimed`, advance `→ wave_<N>_waiting` with `pending_subagents` rebuilt from the not-yet-terminal queue items (`task_id`, `worker_id`, `deadline_at`). Echo and exit.
4. If all queue items are terminal (`complete`, `failed`, or `stale` already parked), advance `→ wave_<N>_recovery`.

#### Wave recovery handler (stage_id: wave_N_recovery)

Reads each completed queue item's result artifact and runs the orchestrator-side gate:

1. Run `shipyard-data queue list --pipeline ship-execute --stage wave_<N>_dispatch`; for every `status: complete` item, read `task.result` as JSON. Required fields: `task`, `status` COMPLETE|BLOCKED, `commit_sha`, `probe_exit_code`, `escalation_code`, `output_tail`, and `capture_file`. A missing, relative, unparseable, or mismatched artifact is a contract violation (treat as BLOCKED). Never regex for `STATUS:` lines from worker chat.
2. **Run the orchestrator-side gate per task from the JSON fields** — follow `dispatching-task-loop`'s "Orchestrator-Side Parsing and Gating" section verbatim: sha `cat-file` verify, PROBE_EXIT:0 check, non-empty output tail, anti-stub-scan on the diff, and on pass `shipyard-data task accept-return <task> sprint=<sprint_id> wave=<N> commit=<sha> --data-dir {{data_dir}}` so the complete-path anchor, `task_dispatch_returned status=complete`, and task `done` mutation happen through the same bundled CLI choke point as sync mode. On `artifact_status: BLOCKED` or JSON `status: BLOCKED`, use the escalation-code routing + the blocked-return `task_dispatch_returned status=blocked` emit. That section is the single orchestrator choke point; do not fork its logic. Missing bundled acceptance reopens the v2.8 orphan vector and starves the terminal gate.
...
Each numbered item is its own stage; the cursor advances stage-by-stage. `/goal` re-enters between stages until a terminal/paused/escalated marker stops it.
...
1. **Rebase + ff-merge, then gate, then tear down — in that order.** Integrate first, verify integration, then remove worktrees. Tearing a worktree down before its branch is merged is exactly what orphaned six verified task commits in the v2.8 incident.
   ...
   b. **Gate before teardown:** `shipyard-data verify-wave-integrated`. Over git ground truth (never the unreliable `worktreeBranch` field) it proves every live `shipyard/wt-*` branch is merged into the working branch AND no `COMPLETE` return commit is dangling.

      It deliberately does **not** require a task's originally-returned sha to be an ancestor of the working branch — the boundary rebase legitimately rewrites those shas, so a returned commit reachable via `shipyard/keep-*` is accepted. See `evaluating-sprint-complete/references/invariants.md` invariant 2 before changing this; requiring ancestry deadlocks every rebased wave.

      Exit 0 → it emits `wave_integration_verified`; proceed to (c). **Exit 3 → HARD STOP:** do NOT remove any worktree (work is safe on the `shipyard/wt-*` branches + `shipyard/keep-*` anchors). Integrate the named branches and re-run the gate once; if that one documented remediation doesn't clear it, follow the `escalating-to-thinker` playbook (trigger: `integration_gate`, subject: the wave / named branches); only if it declines, returns low confidence, or its recommendation also fails, render the gate's stderr output — the named un-integrated branches and dangling commits — verbatim as chat text, then AskUserQuestion. CLI output you saw in a Bash result does not count as shown until echoed.
...
5. **Wave REFACTOR + MUTATE (stage_id: wave_N_refactor)** — **runs only when `execution.refactor_scope: wave`.** Under the default `sprint` scope this is a pass-through: collapse it (`--via wave_<N>_refactor`) and refactor once at `sprint_refactor` instead. When it does run: dispatch a `general-purpose` subagent with an inline wave-refactor prompt (read the combined wave diff, dedupe + rename + add helpers, run a small mutation check, commit if changes). **Model tier (build):** pass `model: <models.build>` from the `!` context / `<SHIPYARD_DATA>/config.md` if non-empty, else OMIT `model:` (inherit session model); never hardcode a literal. **Effort tier (simplifier):** pass `effort: <agent_effort.simplifier>` from config if non-empty (default `low`), else OMIT `effort:`. Not a wave blocker — on failure emit `task_status_changed type=refactor_failed` and advance. **→ `wave_<N>_tests`.**
6. **Wave-scoped tests + single fix (stage_id: wave_N_tests)** — first check the exact-proof reuse from item 4. If the resolved build/test command strings were byte-identical and the build capture exited 0, do not dispatch tests; set `wave_probe_capture` / `wave_probe_exit_code` from the build capture and emit `wave_probe_reused`. Otherwise, if `test_commands.scoped` is configured, substitute and dispatch it per the resolution procedure above; else follow the `dispatching-operational-task` playbook with `test_commands.unit` (today's fallback, unchanged). First time tests run for the wave's merged code unless the exact same command already ran green as the wave build; the operational task streams progress/failures via Monitor. Failure → ONE re-dispatch via `dispatching-task-loop` with the failing-test list as `continuation_note` (`→ wave_<N>_tests_fix_iter_1`); persistent failure emits `task_status_changed type=wave_tests_failed` and advances **to the wave gate — NOT past it**. "Advances" here means the stage graph moves to `wave_<N>_verify` → `wave_<N>_gate`; it does **not** mean the wave passed. At the gate, invariant 3 of the `verifying-wave-completion` playbook requires `wave_probe_exit_code == 0` — a persistently-red suite carries a non-zero `wave_probe_exit_code` and therefore **fails the gate** (ESCALATE: "the regression is real"). Do not read `wave_tests_failed` + advance as permission to ship a red wave; it is permission to let the gate adjudicate. **→ `wave_<N>_verify`.**
...
8. **Wave COMPLETION GATE (stage_id: wave_N_gate)** — follow the `verifying-wave-completion` playbook with `wave_number`, `task_ids`, `data_dir`, `working_branch`, `wave_base_sha`, `wave_head_sha`, `wave_probe_capture`, `wave_probe_exit_code`. It runs the six-invariant composite check with bounded recovery for RECOVERABLE misses and structured escalation otherwise. Only this gate emits `wave_check_passed wave=<N>`, and only on `STATUS: COMPLETE`.

   **Recovery note — the outer cursor must NOT duplicate this bounded recovery.** `verifying-wave-completion` has its OWN internal recovery budget. From the outer cursor's view, `wave_N_gate` is ONE tick that either returns `STATUS: COMPLETE` (advance) or `STATUS: ESCALATED` (`cursor escalate`, AskUserQuestion, do not advance).

9. **Report and continue** — emit `Wave [N]/[M] ✓ [████░░░░] [done]/[total] tasks • → Wave [N+1]`. Under direct invocation: auto-advance into the next wave's dispatch (no pause, no `/clear`, no "continue?"). **→ `wave_<N+1>_dispatch`** (`wave_number=<N+1> iteration=1 stuck_counter=0`), or if `N == M` → **`sprint_refactor`** (`stuck_counter=0`) under the default `execution.refactor_scope: sprint`, else **`sprint_full_build`**.
...
### Step 5: Sprint Completion (stage_ids: sprint_refactor → sprint_full_build → sprint_full_tests → sprint_demo_probes → sprint_complete_gate → terminal_handoff_to_review)

When all waves are done:

0. **Sprint REFACTOR + MUTATE (stage_id: sprint_refactor)** — **runs only under the default `execution.refactor_scope: sprint`**; under `refactor_scope: wave` the per-wave passes already ran, so skip straight from the last `wave_<M>_gate` to `sprint_full_build`.

   **Resolve the scope with the CLI, not by eyeballing config.md:** `shipyard-data resolve-refactor-scope` prints exactly `sprint` or `wave` (same one-source-of-truth pattern as `resolve-isolation`; an unrecognized value resolves to the default, never silently to the other option). The stage graph permits both `wave_gate → sprint_refactor` and `wave_gate → sprint_full_build`, so this read is what decides the edge — capture it once at the last wave's gate and carry it.

   Dispatch a `general-purpose` subagent with the same wave-refactor prompt shape as Step 4 item 5, scoped to the **whole sprint diff** (`sprint_base_sha`..HEAD): dedupe + rename + add helpers, run a small mutation check, commit if changes. Same model/effort tiering rules (`models.build`, `agent_effort.simplifier`; omit the field when config is empty, never hardcode a literal).

   **Why sprint-scoped is the default.** Per-wave refactoring put one LLM agent dispatch per wave directly on the critical path between one wave's builders finishing and the next wave's starting — for a stage explicitly declared "not a wave blocker". One pass also sees the whole diff, so it dedupes across waves that N narrow passes each miss. The trade, for projects that should set `refactor_scope: wave`: one large refactor has worse failure attribution than N small ones, and later waves' builders no longer inherit earlier waves' extractions.

   **This stage is placed BEFORE the full build and suite on purpose** — it is non-blocking on its own failure, so the thing that actually catches a regression it introduces is `sprint_full_build` + `sprint_full_tests` running after it. Do not move it later. On dispatch failure emit `task_status_changed type=refactor_failed` and advance. **→ `sprint_full_build`.**

1. **Full build (stage_id: sprint_full_build)** — if `build_commands.full` configured, follow the `dispatching-operational-task` playbook (catches cross-module compilation errors scoped builds missed). If unconfigured, advance as a pass-through tick (collapsible via `--via`).
...
2. **Full test suite (stage_id: sprint_full_tests)** — resolve `test_commands.unit`, `.integration`, `.e2e` from config.md FIRST, before dispatching anything (a tier with an empty/absent key is skipped). **Check the verification ledger before grouping**: for each resolved tier, run `shipyard-data verify check --key test_commands.<tier> --command "<resolved command>"`. Exit 0 means a fresh proof already exists for this exact tree/command (possibly recorded by `sprint_full_build` because the build command proved that tier); skip dispatching that tier and include it in the summary as `reused`. Exit 3 means stale/missing; include it in the dispatch set. Then **Dedupe identical tiers before dispatching**: group the stale commands by exact string equality; each group maps one-or-more tier names (`unit`/`integration`/`e2e`) onto ONE distinct command, dispatched **once per distinct resolved command**, not once per configured key — a project whose `test_commands.integration` is byte-identical to `.unit` (a real customer's is) must not pay for the same run twice. Note both reuse and dedup in the tick's `--note`, e.g. `--note "unit reused from build proof; integration+e2e identical, deduped to 1 distinct run; distinct runs counted after reuse"`. For each distinct stale command, follow the `dispatching-operational-task` playbook with a synthetic `task_id` of `sprint-full-tests-<n>` (n = 1-based index over the distinct stale commands, in unit→integration→e2e order); the playbook records the absolute logcap capture path in `verify_output`. This is the only time an unproven entire suite tier runs. Persistent failure after the cap → re-dispatch via `dispatching-task-loop` per failing cluster (`→ sprint_tests_fix_iter_1`, K bounded at 1).
   ...
   **Record evidence on a clean pass — per-tier AND aggregate (P1 — closes the redundant re-run at `/ship-review`'s Stage 1a and its quality-gate checks, which check the per-tier keys, not the aggregate one).** When a distinct command's dispatch exits 0, record ONE ledger entry **per tier name that group maps to** — a deduped group covering two tier names (e.g. `unit`+`integration` identical) records TWO entries against the SAME command/exit/capture, one per `--key`, so review doesn't re-run the tier that was deduped away:
   ...
   shipyard-data verify record --key test_commands.<tier> \
       ...
       --exit 0 --capture <absolute logcap path from the operational task's verify_output>
   ```
...
3. **Per-feature user-flow probes (stage_id: sprint_demo_probes)** — the cross-task proof that the feature works *for a user*; catches "all unit tests pass but the integrated feature doesn't work" (the confedit failure mode that motivated this stage in v2.6.0). Unit/integration/e2e-in-CI runs — step 2, just finished — do not exercise the path a user takes; that is what this stage is for. Read SPRINT.md `features:`, then each feature's `user_flow_probe:` (legacy scalar `demo_probe:` still reads as `kind: auto`; note the deprecation once per sprint and move on).
   ...
   ```
   which emits `user_flow_probe_confirmed` and satisfies Invariant 8 exactly as an exit-0 `auto` run does. "Can't check right now" records nothing and pauses the sprint — an unconfirmed flow is not a passing flow, and the cursor is already paused so `/goal` stops.
...
**Patch-task budget (deferral guard).** Before emitting a `patch_task_created source=execute-deviation` event, count how many `patch_task_created` events this sprint has already emitted (the event log is authoritative — `shipyard-context scan-events` / read `.shipyard-events.jsonl`, do NOT count from memory). If that count is at or above `execution.max_patch_tasks` (config.md, default 5), STOP auto-creating patch tasks: render the accumulated patch-task list as chat text and `AskUserQuestion` whether to keep patching, re-plan, or cut scope. Auto-spawning unbounded patch tasks is how a sprint defers its way wide instead of finishing; the cap forces a conscious decision. This mirrors `dispatching-operational-task`'s `operational_tasks.max_patch_tasks` guard, applied to the execute-deviation path.
